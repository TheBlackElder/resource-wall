      ...
      .then((response) => {
        ...
        $(() => {
          ...
          const renderResources = function (resources) {
            ...
            let count = 0;
            for (const resource of resources) {
              console.log('consolelog resourec!!!!',resource.item)
              count++;
              if (count <= 4) {
                let contentType = "";
                if (resource.item.is_video) {
                  const videoId = getId(resource.item.media_url);
                  contentType = `<div class="video-container card-image waves-effect waves-block waves-light">
                  <iframe width="853" height="480" src="//www.youtube.com/embed/${videoId}" frameborder="0" allowfullscreen></iframe>
              </div>`;
                } else {
                  contentType = `<img class="activator" src="${resource.item.media_url}"></img>`;
                }

                const $resource = createResourceElement(resource.item, contentType);
                $("#resources-wrapper").prepend($resource);
              }
            }
          };
